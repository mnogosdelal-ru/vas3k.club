...
def command_help(update: Update, context: CallbackContext) -> None:
    update.effective_chat.send_message(
        "✖️ <b>Я — твой личный бот для Джедайского Клуба Максима Дорофеева</b>\n\n"
...
def private_message(update: Update, context: CallbackContext) -> None:
    ...
    if str(update.effective_user.id) not in set(club_users):
        update.effective_chat.send_message(
            ...
            "<a href=\"https://club.mnogosdelal.ru/user/me/edit/bot/\">кодом из профиля</a> через пробел",
            parse_mode=ParseMode.HTML
        )
    else:
        update.effective_chat.send_message(
...
def main() -> None:
    ...
    # Start the bot (polling mode — Telegram servers can't reach this host directly)
